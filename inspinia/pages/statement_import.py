...
DAY_LABEL_RE = re.compile(r"Day\s+\d+", flags=re.IGNORECASE)
...
PROBLEM_START_RE = re.compile(r"^(?:P\s*)?(?P<number>\d{1,2})[.)]?\s+(?P<statement>.+)$", flags=re.IGNORECASE)
...
class ParsedContestProblemStatement:
    ...
    def problem_code(self) -> str:
        return f"P{self.problem_number}"
...
def _consume_statement_line(
    ...
    *,
    ...
    day_match = DAY_LABEL_RE.fullmatch(line)
    if day_match:
        _flush_problem(state)
        _set_primary_section(state, day_match.group(0).title())
    ...
    elif problem_match := PROBLEM_START_RE.match(line):
        _flush_problem(state)
        state.current_problem_number = int(problem_match.group("number"))
        state.current_statement_lines = [problem_match.group("statement").strip()]
    elif state.current_problem_number is not None:
...
def build_problem_statement_save_preview(
    ...
    existing_rows = ContestProblemStatement.objects.filter(
        contest_year=parsed_import.contest_year,
        contest_name=parsed_import.contest_name,
        problem_number__in=[problem.problem_number for problem in parsed_import.problems],
    )
    existing_by_problem_number = {
        statement.problem_number: statement
    ...
    for parsed_problem in parsed_import.problems:
        existing_statement = existing_by_problem_number.get(parsed_problem.problem_number)
...
def import_problem_statements(
    ...
    for parsed_problem in parsed_import.problems:
        ...
        statement_entry, created = ContestProblemStatement.objects.update_or_create(
            contest_year=parsed_import.contest_year,
            contest_name=parsed_import.contest_name,
            problem_number=parsed_problem.problem_number,
            defaults={
