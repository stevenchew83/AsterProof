...
def _statement_delete_row(statement: ContestProblemStatement) -> dict[str, str]:
    return {
...
def _statement_delete_preview(labels: list[str]) -> str:
    preview_labels = labels[:STATEMENT_DELETE_PREVIEW_LIMIT]
    preview = ", ".join(preview_labels)
    if len(labels) > STATEMENT_DELETE_PREVIEW_LIMIT:
        preview = f"{preview}, and {len(labels) - STATEMENT_DELETE_PREVIEW_LIMIT} more"
    return preview
...
def _completion_is_solved(completion: UserProblemCompletion | None) -> bool:
    return completion is not None and is_completion_status_solved(completion.status)


def _statement_completions_by_statement_id(
    ...
    *,
    ...
    for statement in statements:
        ...
        if statement.linked_problem_id is None:
            continue
        legacy_completion = legacy_completion_by_problem_id.get(statement.linked_problem_id)
        if statement.linked_problem_id in legacy_completion_by_problem_id:
            completion_by_statement_id[statement.id] = legacy_completion
...
def _statement_completion_dates_by_statement_id(
    ...
    *,
    ...
    return {
        statement_id: completion.completion_date
        for statement_id, completion in _statement_completions_by_statement_id(
            statements,
            user=user,
        ).items()
        if _completion_is_solved(completion)
    }
...
def _statement_topic_filter_values(topic_label: str) -> set[str]:
    ...
    return {topic.strip() for topic in topic_values if topic.strip()}


def _filter_statement_queryset(  # noqa: C901
    base,
    *,
    q: str,
    year: str,
    ...
    filtered = annotate_effective_statement_analytics(
        base.select_related("linked_problem"),
    )
    ...
    if year_s:
        ...
        filtered = filtered.filter(contest_year=int(year_s))

    if topic_s:
        filtered = filtered.filter(_eff_topic__in=sorted(_statement_topic_filter_values(topic_s)))
    if confidence_s:
        filtered = filtered.filter(_eff_confidence=confidence_s)
    ...
    try:
        ...
        mx = None
    if mn is not None:
        filtered = filtered.filter(_eff_mohs__gte=mn)
    if mx is not None:
    ...
    if qn:
        for token in qn.split():
            token_query = (
                ...
                | Q(problem_code__icontains=token)
                | Q(day_label__icontains=token)
                | Q(statement_latex__icontains=token)
...
def _problem_statement_list_filter_options(base) -> dict[str, list[str]]:
    years = [
        str(year_value)
        for year_value in base.values_list("contest_year", flat=True).distinct().order_by("-contest_year")
    ]
    analytics_base = annotate_effective_statement_analytics(base)
    topics = sorted(
        {
            display_topic_label(str(topic_value).strip())
            for topic_value in analytics_base.exclude(_eff_topic="")
    ...
    confidences = sorted(
        {
            ...
            for confidence_value in analytics_base.exclude(_eff_confidence="")
            .values_list("_eff_confidence", flat=True)
            .distinct()
            if str(confidence_value).strip()
        },
    )
...
def _user_completion_table_rows(completions: list[UserProblemCompletion]) -> tuple[list[dict], dict[str, list]]:
    ...
    for completion in completions:
        ...
        if completion.status:
            statuses.add(completion.status)
        if completion.confidence:
            confidences.add(completion.confidence)
        table_rows.append(
            {
...
def _admin_completion_listing_rows(
    completions: list[UserProblemCompletion] | None = None,
...
def _admin_solution_listing_rows(
    solutions: list[ProblemSolution] | None = None,
) -> tuple[list[dict], dict[str, int]]:
...
def _completion_board_response_payload(
    *,
    ...
    is_solved: bool,
    completion_date: date | None,
    completion: UserProblemCompletion | None = None,
) -> dict[str, object]:
    problem_label = (
...
def _completion_board_current_completion(
    *,
    user,
    statement: ContestProblemStatement | None,
    problem: ProblemSolveRecord | None,
) -> UserProblemCompletion | None:
    if statement is not None:
    ...
    if problem is None:
        return None
    return UserProblemCompletion.objects.filter(user=user, problem=problem).first()
...
def _completion_board_apply_action(
    *,
    ...
    def defaults_for_completion(completion_date: date | None, *, solved_default: bool) -> dict[str, object]:
        defaults: dict[str, object] = {"completion_date": completion_date}
        if metadata_defaults is not None:
            defaults.update(metadata_defaults)
        if solved_default and not is_completion_status_solved(str(defaults.get("status") or "")):
            defaults["status"] = UserProblemCompletion.Status.SOLVED
        return defaults
    ...
    if action in {"", "toggle"}:
        if completion is None:
            ...
            if statement is not None:
                if statement_completion is not None:
                    ...
                    is_solved = _completion_is_solved(completion)
        ...
        if statement is not None:
            ...
            UserProblemCompletion.objects.update_or_create(
                user=user,
                problem=problem,
                defaults=defaults_for_completion(None, solved_default=True),
            )
        is_solved = True
        completion_date = None
    elif action == "save_details":
        detail_defaults = metadata_defaults or {
        ...
        completion_date = existing_date
        is_solved = is_completion_status_solved(str(detail_defaults.get("status") or ""))
    elif action == "clear":
        if statement is not None:
            if statement_completion is not None:
                statement_completion.delete()
...
def _completion_board_payload(base, *, user, row_limit: int | None) -> dict[str, object]:
    ...
    completion_by_statement_id = _statement_completions_by_statement_id(statements, user=user)
    solved_statement_ids = {
        statement_id
        for statement_id, completion in completion_by_statement_id.items()
        if _completion_is_solved(completion)
    }
    ...
    for contest_name, contest_year in visible_row_keys:
        ...
        for slot_label in problem_columns:
            ...
            completion_date = completion.completion_date if completion is not None and is_solved else None
            if is_solved:
                row_solved_total += 1
                if completion_date is None:
...
def completion_board_toggle_view(request):
    ...
    if has_completion_metadata(request.POST):
        metadata_defaults, metadata_error = completion_metadata_from_post(
            request.POST,
            default_status=UserProblemCompletion.Status.ATTEMPTED
            if action == "save_details"
            else UserProblemCompletion.Status.SOLVED,
        )
    ...
    completion = _completion_board_current_completion(
        user=request.user,
        statement=statement,
        problem=problem,
    )
...
def _completion_quick_update_row(
    ...
    *,
    ...
    completion_date = completion.completion_date if completion is not None and is_completed else None
...
def _completion_quick_update_result_summary(
    *,
    ...
    if not matching_total_is_exact:
        if is_capped:
            return f"Showing first {visible_total} matching rows"
        return f"{visible_total} matching row{'s' if visible_total != 1 else ''}"
...
def user_activity_dashboard_view(request):
    ...
    solved_completions = [completion for completion in completions if _completion_is_solved(completion)]
    today = timezone.localdate()
    dated_completion_dates = [
        completion.completion_date
        for completion in solved_completions
        if completion.completion_date is not None
    ]
    unknown_completion_total = len(solved_completions) - len(dated_completion_dates)
    table_rows, filter_options = _user_completion_table_rows(completions)
    statement_completion_rows = _user_statement_completion_rows(solved_completions)
    statement_completion_heatmap = _contest_year_heatmap_payload(
        statement_completion_rows,
        value_key="completed_count",
    )
    topic_mohs_completion_heatmap = _user_topic_mohs_completion_heatmap_payload(solved_completions)
    ...
    if selected_status:
        filtered_table_rows = [
            row for row in filtered_table_rows if row["status"] == selected_status
        ]
    if selected_confidence:
        filtered_table_rows = [
            row for row in filtered_table_rows if row["confidence"] == selected_confidence
        ]
    if selected_contest:
        filtered_table_rows = [
    ...
    context = {
        ...
        "activity_topic_mohs_completion_stats": {
            ...
            "completion_total": len(solved_completions),
...
def user_solution_record_list_view(request):
    ...
    solution_queryset = ProblemSolution.objects.select_related("author", "problem").annotate(
        block_count=Count("blocks", distinct=True),
    )

    if selected_contest:
        solution_queryset = solution_queryset.filter(problem__contest=selected_contest)
    if selected_user:
        solution_queryset = solution_queryset.filter(author__email=selected_user)
    if selected_status:
        solution_queryset = solution_queryset.filter(status=selected_status)
    if search_query:
        for token in search_query.split():
            token_query = (
                Q(author__name__icontains=token)
                | Q(author__email__icontains=token)
                | Q(problem__contest__icontains=token)
                | Q(problem__problem__icontains=token)
                | Q(problem__contest_year_problem__icontains=token)
                | Q(problem__topic__icontains=token)
                | Q(title__icontains=token)
                | Q(status__icontains=token)
            )
            if token.isdigit():
                token_query |= Q(problem__year=int(token))
            solution_queryset = solution_queryset.filter(token_query)

    solution_filtered_total = solution_queryset.count()
    solution_rows, _solution_stats = _admin_solution_listing_rows(
        list(solution_queryset.order_by("-updated_at", "-id")[:ADMIN_TABLE_LATEST_LIMIT]),
    )
    solution_visible_total = len(solution_rows)

    solution_filter_options = {
        "contests": sorted({row["contest"] for row in solution_rows if row["contest"]}),
        "statuses": [
            status
            for status, _label in ProblemSolution.Status.choices
...
def problem_statement_list_view(request):
    ...
    statement_filtered_total = filtered_statements.count()
    visible_statement_rows = (
        _statement_table_rows(
            list(
                filtered_statements.order_by("-updated_at", "-id")[:ADMIN_TABLE_LATEST_LIMIT],
            ),
            user=request.user,
        )
        if statement_filtered_total
        else []
    )
    visible_statement_rows = _json_script_safe(visible_statement_rows)
    _ = json.dumps(visible_statement_rows, cls=DjangoJSONEncoder, allow_nan=False)
    statement_visible_total = len(visible_statement_rows)

    filter_options = _problem_statement_list_filter_options(base)
    statement_datatable_rows = visible_statement_rows
    ...
    context = {
        ...
        "statement_table_rows": visible_statement_rows,
        "statement_datatable_rows": statement_datatable_rows,
        "statement_filtered_total": statement_filtered_total,
        ...
        if form.is_valid():
            ...
            if stale_selection:
                ...
                messages.success(
                    ...
                    f"{_statement_delete_preview(selected_statement_labels)}.",
                )
...
def problem_list_redirect_view(request):
    """Temporary compatibility redirect from the retired /problems/ page."""
...
def technique_gap_benchmark_view(request):  # noqa: C901, PLR0912, PLR0915
    ...
    elif request.method == "POST" and action == "mark_reviewed":
...
def _export_statement_subtopic_workbook_response() -> HttpResponse:
    workbook_bytes = build_statement_subtopic_export_workbook_bytes()
    timestamp = timezone.localtime().strftime("%Y%m%d-%H%M%S")
    response = HttpResponse(workbook_bytes, content_type=XLSX_CONTENT_TYPE)
    response["Content-Disposition"] = (
        f'attachment; filename="asterproof-statement-subtopics-{timestamp}.xlsx"'
    )
    return response
...
def problem_statement_metadata_view(request):
    ...
    if request.method == "GET" and (
        export_handler := _statement_metadata_export_handler(request.GET.get("action"))
    ):
