    ...
    for (let i = 0; i < countriesList.length; i += 2) {
        ...
        for (let j = 0; j < 2; j++) {
            ...
            if (country && country.code && country.name) {
                ...
                const imgPath = `/static/images/flags/${code}.svg`;

                row.innerHTML += `
                    <td>
                        <img src="${imgPath}" alt="${country.name}" height="18" class="rounded"
                            onerror="this.onerror=null; this.src='/static/images/flags/unknown.svg'; this.alt='Not Found'; this.classList.add('border', 'border-danger');">
                    </td>
                    <td>${country.name}</td>
