...
**Cursor / agents:** Use the relevant sections for dashboard/admin UI changes, including UI generated by Python. The [Cursor rule](../.cursor/rules/inspinia-dashboard.mdc) targets template and theme source files; it is not an always-on rule.
...
- If you change variables or SCSS, **rebuild** assets (`npm run build`; see `README.md` for dependency setup) so `app.min.css` stays the single source of truth in production templates.
